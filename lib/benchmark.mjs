...
export function matrixFromTsv(tsv) {
  ...
  const xValues = numericWords
    .map((word) => word.left + word.width / 2)
    .sort((a, b) => a - b);
  const threshold = Math.max(
    60,
    (Math.max(...xValues) - Math.min(...xValues)) / 10,
  );
  const clusters = [];
  ...
  const clusteredCenters = clusters
  ...
  const firstRowY = preliminaryValueLines[0].centerY;
  const headerLine = lines
    .filter((line) => line.centerY < firstRowY - yTolerance)
    .map((line) => ({
      ...line,
      score:
        line.words.filter((word) => /\d/.test(word.text)).length * 10 +
        line.words.length,
    }))
    .sort((a, b) => b.score - a.score)[0];
  const headerWords = headerLine?.words || [];
  const headerMatchedCenters = [
    ...new Set(
      headerWords.map((word) => {
        const wordCenter = word.left + word.width / 2;
        return [...clusteredCenters].sort(
          (a, b) => Math.abs(a - wordCenter) - Math.abs(b - wordCenter),
        )[0];
      }),
    ),
  ].sort((a, b) => a - b);
  const columnCenters =
    headerMatchedCenters.length >= 2 ? headerMatchedCenters : clusteredCenters;
  ...
  const rows = valueLines.map((line, rowIndex) => {
    ...
    if (!labelWords.length) {
      labelWords = lines
        .filter(
          (candidate) =>
            Math.abs(candidate.centerY - line.centerY) <= yTolerance * 2.5,
        )
        .sort(
          (a, b) =>
